...
class Gestion {
    ...
    fun crearEvento() {
        ...
        val fechaEvento = readln() //Fecha en la que se va a realizar el evento
        //TODO HACER METODO PARA COMPROBAR LA FECHA

        println("¿A que hora se realizará (hh:mm)?")
        val horaEvento = readln() //Hora del evento

        println("Dime cuantas horas dura el evento")
        val duracion = readln().toInt()

        val ubicacionCreada = crearUbicacion() //Llamo a la función de crear la ubicacion del evento
        ...
        //Crear organizador o no
        println("¿Quieres crear un organizador nuevo? (Si-No)")
        val nuevoOrganizador = readln().lowercase()

        val organizador: Organizador

        if (nuevoOrganizador == "si") {
            ...
            if (posicionNombre == -1) {
                ...
                organizador = crearOrganizador() //Si el organizador no se encuentrea, crea uno.
            } else {
                //Caso de que si haya encontrado organizador
                organizador = listaOrganizadores[posicionNombre]
            }
        }
        ...
        var categoria = readln().lowercase()
        ...
        if (categoria !in categoriasDisponibles) {
            println("La categoria ($categoria), no es una categoria válida. Por defecto se atribuirá (conferencia)")
            categoria = "conferencia" //Atribuyo conferencia por defecto
        }

        //Creacion del evento
        val eventoCreado =
            Evento(nombreEvento, fechaEvento, horaEvento, duracion, ubicacionCreada, isOnline, organizador, categoria)
    ...
    fun filtrarEventoCategoria(): String {
        ...
        return "Los eventos de la categoria $categoriaFiltrar son ${eventosFiltrada.joinToString { it }}"
    }
